...
El proyecto se gestionó con un enfoque ágil basado en \textbf{Scrum}. Se definieron sprints quincenales con \textit{product backlog} priorizado en torno a: (i) captura conversacional por WhatsApp, (ii) persistencia y gobierno de datos, (iii) cálculo estandarizado de puntajes DASS{-}21 y agregados semanales, (iv) analítica y \textit{dashboards}, y (v) robustez, monitoreo y seguridad. Las responsabilidades principales fueron: \textbf{Johan} (bot, integración y despliegue) y \textbf{Diego} (analítica con Polars, \textit{feature engineering} y clustering). Los \textit{Definition of Done} incluyeron pruebas unitarias, validaciones de rangos, documentación y registro en bitácora de cambios.
...
El sistema es \textbf{preventivo}, no clínico. Se incluye \textit{disclaimer} y \textbf{consentimiento informado}. La analítica evita decisiones automáticas individualizantes; los \textbf{alertamientos} (si se activan) deben insertarse en protocolos universitarios de apoyo. Se resguardan datos con cifrado en tránsito y en reposo, control de accesos y auditoría de uso. Las métricas y reportes agregados reducen el riesgo de identificación re\-identificable.
...
Se condujo un \textbf{piloto controlado con datos simulados} que replica la estructura esperada en operación (150 estudiantes, 8 semanas). Se obtuvieron los siguientes resultados:

\begin{itemize}[leftmargin=*]
  \item \textbf{Adherencia}: la tasa de respuesta semanal media fue del 74\% (DE=8{,}2\%), con estabilidad de participación a partir de la semana 3.
  \item \textbf{Desempeño técnico}: el p95 del tiempo de procesamiento de \textit{webhook} se mantuvo por debajo de 800\,ms; no se observaron caídas de disponibilidad en el servicio receptor durante las pruebas de carga moderada.
  \item \textbf{Calidad de datos}: el 96{,}5\% de registros pasó las validaciones de rango y completitud; los faltantes se concentraron en ventanas nocturnas, lo que sugiere ajustar ventanas de envío.
  \item \textbf{Tendencias}: se observaron incrementos transitorios en \textit{estrés} y \textit{ansiedad} en semanas marcadas como de parciales; los deltas semanales retornaron a línea base una semana posterior al evento académico.
  \item \textbf{Clustering}: el \textbf{método del codo} sugirió $k \in \{3,4\}$; con $k{=}3$ emergieron perfiles: (C1) \textit{baja carga y estable}, (C2) \textit{moderada con picos en evaluaciones}, (C3) \textit{alta persistente}. La separación (silhouette promedio) fue consistente con las diferencias en deltas y varianza intra-semana.
\end{itemize}

En conjunto, el sistema \textbf{cumplió} los objetivos de captura automatizada, cálculo estandarizado de puntajes, agregación semanal y generación de analítica descriptiva con \textbf{robustez operativa}. Estos hallazgos serán replicados con datos reales en un despliegue acotado y con aprobación institucional.

\section*{V. CONCLUSIONES Y TRABAJO A FUTURO}
El proyecto \textbf{EmotiTrack UDG} demostró la viabilidad de un \textbf{bot de entrevista} semanal integrado a WhatsApp para monitorear \textbf{estrés, ansiedad y depresión} con el DASS{-}21, manteniendo \textbf{calidad de datos}, \textbf{escalabilidad} y \textbf{tiempos de respuesta} adecuados. La analítica basada en Polars y \textbf{clustering} permitió identificar \textbf{perfiles diferenciados} y \textbf{semanas críticas} asociadas a la carga académica.

Como \textbf{limitaciones}, el piloto utilizó \textbf{datos simulados}; además, se requiere evaluar sesgos de no respuesta y validar umbrales de alerta en población local. Como \textbf{trabajo futuro}: (i) despliegue gradual con cohortes reales y consentimiento, (ii) tableros con \textbf{alertas tempranas} y rutas de derivación, (iii) experimentos \textbf{A/B} de mensajes y horarios para mejorar adherencia, (iv) validación externa de \textbf{severidades} y \textbf{reglas} con expertos, (v) fortalecimiento de \textbf{gobierno de datos} (retención, auditoría) y \textbf{seguridad} (segregación de ambientes, \textit{secret management}), y (vi) análisis de \textbf{impacto} sobre métricas de bienestar y deserción.
