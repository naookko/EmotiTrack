...
\subsection*{A. Justificación del uso de DASS{-}21 frente a alternativas}
...
Este módulo vincula la \textbf{arquitectura de despliegue} con los objetivos de disponibilidad y resiliencia. El stack se ejecuta en una instancia \textbf{AWS EC2} containerizada con \textbf{Docker}, donde cada servicio (listener FastAPI, workers de analítica, listener de webhooks) corre en contenedores aislados, con sus dependencias y puertos mínimos publicados\cite{docker_docs}. Esta estrategia facilita \textbf{reproducibilidad} (mismas imágenes en desarrollo y producción), \textbf{escalabilidad horizontal} (réplicas adicionales por servicio durante picos) y contención de fallos: si un worker se cae, el resto del sistema continúa operando.
\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{../img/DiagramainfraestructuraEmotitrack.drawio.png}
  \caption{Topología de despliegue: Nginx + Docker en EC2 con contenedores aislados y monitoreo.}
  \label{fig:infra_diagram}
\end{figure}

El \textbf{reverse proxy Nginx} termina TLS y enruta tráfico a los contenedores internos. Los certificados \textbf{SSL/TLS} provienen de \textbf{Let's Encrypt} mediante \textbf{Certbot}, con renovación automática cada 90 días\cite{certbot_docs}. Nginx fuerza redirección 80$\rightarrow$443 y sólo expone el puerto necesario hacia FastAPI, de forma que los microservicios nunca quedan abiertos a Internet.

La \textbf{capa de red} sigue el principio de privilegios mínimos: el \textbf{security group} de EC2 abre únicamente 22/TCP (administración), 80/TCP (validación Certbot) y 443/TCP (webhooks). Todos los puertos internos de contenedores permanecen cerrados al exterior, lo que reduce la superficie de ataque y se alinea con las guías de AWS\cite{aws_security_groups}. Cada webhook de WhatsApp llega cifrado a Nginx, se reenvía a FastAPI y dispara tareas asíncronas (por \textit{background tasks} o colas internas) que procesan los datos en paralelo sin bloquear la respuesta.

Los \textbf{jobs de analítica} (Polars, K{-}Means) corren en contenedores dedicados programados vía cron o servicios supervisados; comparten una red interna con MongoDB y publican resultados en almacenamiento persistente. Se monitorean \textbf{timeouts}, \textbf{reintentos exponenciales}, \textbf{idempotencia} de operaciones y \textbf{dead-letter queues} para eventos fallidos, acompañados de \textbf{logging estructurado}, métricas de p95 de webhook $<1\,\mathrm{s}$ y trazas distribuidas. Este diseño modular asegura que la plataforma sea \textbf{segura} (TLS + puertos mínimos), \textbf{escalable} (réplicas de contenedores) y \textbf{tolerante a fallos} (aislamiento + reinicios automatizados), cumpliendo con los requerimientos del módulo.

\subsection*{Módulo Justificación de Cómputo Flexible (softcomputing)}
El proyecto aplica \textbf{métodos flexibles} para manejar la variabilidad y el ruido inherentes a los datos de bienestar emocional. Se trabaja con \textbf{DASS{-}21} y cortes \textbf{semanales} que estandarizan subescalas (depresión, ansiedad, estrés) y validan completitud/rangos antes de alimentar la \textbf{ingeniería de características}, la cual agrega contexto académico (materias, semanas de parcial, cargas simultáneas), marcadores de respuesta tardía y deltas entre check-ins consecutivos descritos en \texttt{kmeans\_full\_markdown.md}.

Para segmentar perfiles se eligió \textbf{K{-}Means} porque es \textbf{intuitivo} (cada centroide representa un perfil promedio comprensible por psicólogos), \textbf{eficiente} para recalcular con lotes semanales y \textbf{consistente} al mantener el mismo \(k\) y comparar centroides entre semanas. El algoritmo, propuesto por MacQueen y formalizado por Lloyd, Hartigan y Wong\cite{macqueen1967,lloyd1982,hartiganwong1979}, minimiza la inercia
\[
J = \sum_{i=1}^{K} \sum_{x \in C_i} \lVert x - \mu_i \rVert^2
\]
y su implementación en \textbf{Scikit-Learn}\cite{sklearn} permite controlar inicializaciones (K{-}Means\texttt{++}), reproducibilidad y métricas (\textit{inertia}, \textit{silhouette}). La selección de \(k\) se respalda con el método del codo y \textit{cross-validation} semanal sobre el dataset simulado, además del monitoreo de drift entre centroides consecutivos.

Se documenta también la \textbf{complejidad} casi lineal \(O(n \times k \times t)\) que vuelve viable recalcular clústeres con 100 respuestas por semana, así como las suposiciones de \textbf{formas globulares} y necesidad de \textbf{escalamiento} para variables en distinta magnitud (implementado con normalización min-max previa). Se evaluaron alternativas como \textbf{clustering jerárquico} y \textbf{DBSCAN}; sin embargo, requieren mayor calibración (dendrogramas o pares \(\varepsilon\)/MinPts) y generan clústeres menos estables entre semanas con tamaños de muestra reducidos, lo que dificulta su comunicación en tablero. K{-}Means ofrece un \textbf{equilibrio entre simplicidad operativa y valor interpretativo}: produce 3--5 segmentos repetibles, permite etiquetar alertas por proximidad a centroides y se integra a los dashboards (Filament/Streamlit) con indicadores de tendencia, drift e inercia. La capa de softcomputing se completa con reglas \textbf{CART} para validar los segmentos, pruebas de sensibilidad con semillas distintas y monitoreo ético (el modelo sólo sugiere acciones preventivas, no diagnósticos clínicos).
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../img/dass_total_distribution.png}
  \caption{Distribución de Puntuaciones Totales (DASS-21) en la semana analizada.}
  \label{fig:dass_total_dist}
\end{figure}

La distribución del puntaje total permite evaluar \textbf{tendencia central, dispersión y colas}, indicadores clave para dimensionar el \textbf{riesgo agregado} de la cohorte y detectar \textbf{asimetrías} (p.\,ej., acumulación en rangos moderados o presencia de una cola derecha con valores altos). Esta lectura sustenta umbrales de alerta y la necesidad de intervenciones a nivel grupo cuando se observan desplazamientos respecto a semanas previas.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../img/dass_subscales_boxplot.png}
  \caption{Distribución de subescalas DASS-21 (Estrés, Ansiedad, Depresión).}
  \label{fig:dass_subscale_box}
\end{figure}

El boxplot comparativo revela \textbf{dispersión, mediana y valores atípicos} por subescala, diferenciando patrones entre \textbf{estrés, ansiedad y depresión}. Esta comparación es útil para \textbf{priorizar acciones}: por ejemplo, si la subescala de estrés muestra mayor IQR o más outliers, conviene dirigir primero recursos a \textit{manejo de carga académica} antes que a intervenciones focalizadas en ansiedad o depresión. Además, la estabilidad de estas formas semana a semana es un \textbf{indicador de deriva} del sistema.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../img/top15_total_score.png}
  \caption{Top 15 estudiantes por puntuación total (DASS-21).}
  \label{fig:top15_total}
\end{figure}

El \textit{Top 15} identifica casos con \textbf{mayor carga sintomática} en el corte semanal, lo que habilita \textbf{rutas de seguimiento} bajo las políticas de acceso y seudonimización del panel. Su uso es \textbf{preventivo y no clínico}: sirve para priorizar \textbf{contacto proactivo} o \textbf{apoyos académicos} cuando los puntajes altos son \textbf{persistentes} en múltiples semanas o coinciden con subidas en la severidad. En el tablero, esta lista debe restringirse por \textbf{rol/permisos} y operar sobre \textbf{identificadores seudónimos}, evitando datos nominativos.
...
\begin{thebibliography}{12}
  ...
  J.~MacQueen, ``Some methods for classification and analysis of multivariate observations,'' in \emph{Proc. Fifth Berkeley Symposium on Mathematical Statistics and Probability}, vol.~1, 1967, pp.~281--297.

  \bibitem{lloyd1982}
  S.~P. Lloyd, ``Least squares quantization in PCM,'' \emph{IEEE Transactions on Information Theory}, vol.~28, no.~2, pp.~129--137, 1982.

  \bibitem{hartiganwong1979}
  J.~A. Hartigan and M.~A. Wong, ``Algorithm AS 136: A k-means clustering algorithm,'' \emph{Journal of the Royal Statistical Society. Series C (Applied Statistics)}, vol.~28, no.~1, pp.~100--108, 1979.
  ...
  \bibitem{docker_docs}
  Docker, ``Container isolation and security best practices,'' \url{https://docs.docker.com/} (último acceso: Nov. 2025).

  \bibitem{certbot_docs}
  Electronic Frontier Foundation, ``Certbot documentation for Nginx and Let's Encrypt,'' \url{https://certbot.eff.org/} (último acceso: Nov. 2025).

  \bibitem{aws_security_groups}
  Amazon Web Services, ``Security groups for your VPC,'' \url{https://docs.aws.amazon.com/vpc/latest/userguide/VPC_SecurityGroups.html} (último acceso: Nov. 2025).
\end{thebibliography}
\end{document}
