...
def read_last_week_entry():
    """Lee la última semana registrada en week_history.txt.
    Devuelve (start_date, end_date) como date o (None, None) si no existe/está vacío.
    """
...
# -------------------------------
# Política de semana a ejecutar:
# - Si existe una semana previa en week_history.txt, usar la siguiente (prev_start + 7 días).
# - Si no existe la siguiente semana en el historial, usar una fecha inicial configurable.
#   Establecer con la variable de entorno INITIAL_WEEK_START (YYYY-MM-DD) o
#   como respaldo START_WEEK_DATE para compatibilidad.

INITIAL_WEEK_START_STR = (
    os.environ.get("INITIAL_WEEK_START")
    or os.environ.get("START_WEEK_DATE")  # compatibilidad previa
    or "2025-09-05"  # valor por defecto si no se configura
).strip()

try:
    initial_week_start = date.fromisoformat(INITIAL_WEEK_START_STR)
except ValueError as exc:
    raise SystemExit(
        f"INITIAL_WEEK_START must be YYYY-MM-DD, got '{INITIAL_WEEK_START_STR}'."
    ) from exc
...
if last_start and last_end:
    ...
    week_start_date = initial_week_start
...
run_datetime = datetime.combine(week_end_date, datetime.max.time()).replace(microsecond=0)
